function [face1, face2, face3,face4, rmse1, rmse2, rmse3, rmse4] = problemOne(im_array, index1, index2, index3, index4)
    im_array2 = im2double(im_array); 
    num_training_images = 60;
    num_basis_vectors = 40;
    
    images = im_array2(1:num_training_images,:); %choose 60 images
...
[    V,D] = eigs(C, size(C,1)); %eigenvalue/eigenvector of AAt
    ...
    imshow(reshape(face1(1,:),[128,128]));
    imshow(reshape(face2(1,:),[128,128]));
    imshow(reshape(face3(1,:),[128,128]));
    imshow(reshape(face4(1,:),[128,128]));
